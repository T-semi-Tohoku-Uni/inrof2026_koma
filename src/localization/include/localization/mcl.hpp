...
namespace koma {
    ...
    class MCL: public rclcpp::Node {
        ...
        private:
            ...
            void lidarpose2uv(double range, double theta, geometry_msgs::msg::Pose pose, double *x_odom, double *y_odom, int *u, int *v);
            void xy2uv(std::double_t x, std::double_t y, int32_t *u, int32_t *v);
            double randNormal(double n) { return (n * sqrt(-2.0 * log((double)rand() / RAND_MAX)) * cos(2.0 * M_PI * rand() / RAND_MAX)); }
