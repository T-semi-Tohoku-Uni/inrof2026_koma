...
koma::MCL::MCL(const rclcpp::NodeOptions & options): Node("mcl", options) {
    ...
    // map settings
    std::filesystem::path map_path = std::filesystem::path(this->declare_parameter<std::string>("map_path"));
    ...
    particle_marker_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "cloud", 
...
void koma::MCL::update_particles() {
    ...
    for (size_t i = 0; i < this->particles_.size(); i++ ) {
        ...
        // update position
        update_particle_pose.position.x = particle_pose.position.x + std::cos(theta)*dx - std::sin(theta)*dy;
        update_particle_pose.position.y = particle_pose.position.y + std::sin(theta)*dx + std::cos(theta)*dy;
...
void koma::MCL::lidarpose2uv(double range, double theta, geometry_msgs::msg::Pose pose, double *x_odom, double *y_odom, int *u, int *v) {
...
std::vector<double> koma::MCL::caculate_likelihood_field_model(geometry_msgs::msg::Pose particle_pose) {
...
void koma::MCL::laser_scan_callback(sensor_msgs::msg::LaserScan::SharedPtr msgs) {
    scan_ = msgs;
}
