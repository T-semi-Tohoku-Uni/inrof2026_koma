    ...
    class MCL: public rclcpp::Node {
        public:
            explicit MCL(const rclcpp::NodeOptions & options = rclcpp::NodeOptions()): Node("mcl_node", options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_) {
                ...
                subCmdVel_ = create_subscription<nav_msgs::msg::Odometry>(
                    "odom", cmdVelQos, std::bind(&MCL::cmdVelCallback, this, std::placeholders::_1)
                );
        ...
        private:
            ...
            void cmdVelCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
                cmdVel_ = std::make_shared<geometry_msgs::msg::Twist>();

                cmdVel_->linear.x = msg->twist.twist.linear.x;
                cmdVel_->linear.y = msg->twist.twist.linear.y;
                cmdVel_->linear.z = msg->twist.twist.linear.z;
                cmdVel_->angular.z = msg->twist.twist.angular.z;
            }
            ...
            void loop() {
                // 並進速度・回転速度を取得
                RCLCPP_INFO(this->get_logger(), "rjhefgojrsaefojrafjere");
                if (!cmdVel_) {
                    return;
                }
                ...
                RCLCPP_INFO(this->get_logger(), "fkjoperjfoirejfioreijfer");
            ...
            rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subCmdVel_;
