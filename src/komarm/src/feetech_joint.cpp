...
koma::Serial::Serial(const char* device_name) {
...
koma::FeetechJointManager::FeetechJointManager(const rclcpp::NodeOptions & options) :
    ...
    serial_port(
      this->declare_parameter<std::string>("port", "platform-fd500000.pcie-pci-0000:01:00.0-usb-0:1.1.1:1.0").c_str()
    )
{
    cur_joint_state_pub_ = this->create_publisher<sensor_msgs::msg::JointState> (
