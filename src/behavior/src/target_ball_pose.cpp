...
koma::TargetBallPosition::TargetBallPosition(
    ...
    std::shared_ptr<koma::BTNode> ros_node
): 
...
BT::NodeStatus koma::TargetBallPosition::tick() {
    std::optional<inrof2026_koma_type::srv::BallPosition::Response> target_ball_position = this->ros_node_->target_ball_position();
    ...
    RCLCPP_INFO(ros_node_->get_logger(), "Ball found (x, y) = (%lf, %lf)", target_ball_position->pose_stamped.pose.position.x, target_ball_position->pose_stamped.pose.position.y);
