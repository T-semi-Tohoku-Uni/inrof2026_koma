...
koma::PathPlanner::PathPlanner(const rclcpp::NodeOptions & options): Node("path_planner", options) {
    ...
    goal_pose_srv_ = this->create_service<inrof2026_koma_type::srv::PoseStamped>(
        "goal_pose",
        std::bind(&PathPlanner::goal_pose_callback, this, std::placeholders::_1, std::placeholders::_2)
    );
...
void koma::PathPlanner::goal_pose_callback(const std::shared_ptr<inrof2026_koma_type::srv::PoseStamped::Request> request, std::shared_ptr<inrof2026_koma_type::srv::PoseStamped::Response> response) {
    RCLCPP_INFO(this->get_logger(), "Received goal pose: x=%f, y=%f", request->pose_stamped.pose.position.x, request->pose_stamped.pose.position.y);

    waypoint_array_.push_back(std::make_pair(request->pose_stamped.pose.position.x, request->pose_stamped.pose.position.y));

    std::vector<std::pair<double, double>> path;
    std::pair<double, double> start_point = std::make_pair(robot_pose_.position.x, robot_pose_.position.y);
...
std::vector<std::pair<double, double>> koma::PathPlanner::generator(std::pair<double, double> start_point, std::pair<double, double> goal_point) {
    ...
    std::vector<std::vector<double>> distances(
        this->map_height_, std::vector<double>(this->map_width_, std::numeric_limits<double>::infinity())
    );
...
void koma::PathPlanner::waypoint_callback(const std::shared_ptr<inrof2026_koma_type::srv::PoseStamped::Request> request, std::shared_ptr<inrof2026_koma_type::srv::PoseStamped::Response> response) {
    waypoint_array_.push_back(std::make_pair(request->pose_stamped.pose.position.x, request->pose_stamped.pose.position.y));
    RCLCPP_INFO(this->get_logger(), "Received waypoint: x=%f, y=%f", request->pose_stamped.pose.position.x, request->pose_stamped.pose.position.y);
    response->success = true;
}
