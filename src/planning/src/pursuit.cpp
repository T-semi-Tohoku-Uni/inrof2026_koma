...
koma::PIDController::PIDController(double Kp, double Ki, double Kd, double dt, std::function<double(double)> normalize_func)
...
koma::Pursuit::Pursuit(const rclcpp::NodeOptions & options) : Node("pursuit", options) {
    ...
    control_timer_ = this->create_wall_timer(
        100ms,
        std::bind(&koma::Pursuit::control_loop, this)
    );
}
...
rclcpp_action::GoalResponse koma::Pursuit::handle_goal(
    ...
    if (goal_handle_) {
        RCLCPP_WARN(this->get_logger(), "Received a new goal request, but there is already an active goal. Rejecting.");
...
rclcpp_action::CancelResponse koma::Pursuit::handle_cancel(const std::shared_ptr<rclcpp_action::ServerGoalHandle<inrof2026_koma_type::action::Pursuit>> goal_handle) {
    goal_handle_.reset();
    return rclcpp_action::CancelResponse::ACCEPT;
}

void koma::Pursuit::handle_accepted(const std::shared_ptr<rclcpp_action::ServerGoalHandle<inrof2026_koma_type::action::Pursuit>> goal_handle) {
...
void koma::Pursuit::control_loop() {
    ...
    linear_speed.linear.x = cos(pose_theta) * linear_speed_cmd_x + sin(pose_theta) * linear_speed_cmd_y;
    linear_speed.linear.y = -sin(pose_theta) * linear_speed_cmd_x + cos(pose_theta) * linear_speed_cmd_y;
