...
public:
  ...
  bool set_reverse(int servo_id, bool reverse);
...
private:
  ...
  uint8_t make_checksum(uint8_t tx[], size_t size);
  uint16_t make_u16(uint8_t low, uint8_t high)
  {
    return static_cast<uint16_t>(low) | (static_cast<uint16_t>(high) << 8);
  }
  bool write_packet(uint8_t tx[], size_t size);
  std::string bytes_to_hex(const std::vector<uint8_t> & buffer, size_t size);
