...
void koma::FeetechState::print()
{
  ...
  RCLCPP_INFO(
    logger, "Max Temperature Limit : %u degC", static_cast<unsigned>(max_temperature_limit));
  RCLCPP_INFO(
    logger, "Max Voltage Limit     : %.1f V raw=%u", static_cast<double>(max_voltage_limit) / 10.0,
    static_cast<unsigned>(max_voltage_limit));
  RCLCPP_INFO(
    logger, "Min Voltage Limit     : %.1f V raw=%u", static_cast<double>(min_voltage_limit) / 10.0,
  ...
  RCLCPP_INFO(
    logger, "Present Voltage       : %.1f V raw=%u", static_cast<double>(present_voltage) / 10.0,
    static_cast<unsigned>(present_voltage));
  RCLCPP_INFO(
    logger, "Present Temperature   : %u degC", static_cast<unsigned>(present_temperature));
...
int koma::FeetechSerial::open_serial(const char * device_name)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Serial opened: %s @ 1000000, 8N1, raw", device_name);
  return fd;
}
...
bool koma::FeetechSerial::try_read_write_ack(int servo_id)
{
  ...
  if (n > 0) {
    ...
    RCLCPP_ERROR(logger, "Serial read failed while reading WRITE ACK: %s", std::strerror(errno));
    rx_buffer_.clear();
    return false;
  }
  ...
  auto header_it =
    std::search(rx_buffer_.begin(), rx_buffer_.end(), std::begin(header), std::end(header));
  ...
  if (rx_id != servo_id_uint8_t) {
    RCLCPP_WARN(
      logger, "Unexpected ACK servo id: received=%u expected=%u", static_cast<unsigned>(rx_id),
  ...
  if (length != 0x02) {
    RCLCPP_WARN(
      logger, "Unexpected packet while waiting WRITE ACK: length=%u head=%s",
  ...
  if (received_checksum != expected_checksum) {
    RCLCPP_WARN(
      logger, "WRITE ACK checksum mismatch: received=0x%02X expected=0x%02X", received_checksum,
      expected_checksum);
    return false;
  }

  if (error != 0x00) {
    RCLCPP_WARN(logger, "WRITE ACK returned error: 0x%02X", error);
    return false;
  }
...
bool koma::FeetechSerial::try_read_state_response(int servo_id, koma::FeetechState & state)
{
  ...
  if (!rx_buffer_.empty()) {
    RCLCPP_INFO(
      logger, "RX buffer size=%zu head=%s", rx_buffer_.size(),
      bytes_to_hex(rx_buffer_, std::min<size_t>(rx_buffer_.size(), 32)).c_str());
  }
  ...
  if (rx_id != servo_id_uint8_t) {
    RCLCPP_WARN(logger, "Unexpected servo id: received=%u expected=%u", rx_id, servo_id);
  ...
  if (length != read_size + 2) {
    RCLCPP_WARN(
      logger, "Unexpected response length: received=%u expected=%u", length,
  ...
  if (error != 0x00) {
    RCLCPP_WARN(logger, "Servo returned error: 0x%02X", error);
  ...
  if (received_checksum != expected_checksum) {
    RCLCPP_WARN(
      logger, "Checksum mismatch: received=0x%02X expected=0x%02X", received_checksum,
      expected_checksum);
...
bool koma::FeetechSerial::read_all_state(int servo_id, koma::FeetechState & state)
{
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("rclcpp"), "TX: %02X %02X %02X %02X %02X %02X %02X %02X", tx[0], tx[1],
    tx[2], tx[3], tx[4], tx[5], tx[6], tx[7]);
  ...
  while (received < expected_rx_size) {
    ssize_t n = ::read(serial_fd_, rx + received, expected_rx_size - received);
    ...
    if (n > 0) {
      ...
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Serial read failed: %s", std::strerror(errno));
        break;
      }
    }
  ...
  if (received < expected_rx_size) {
    RCLCPP_WARN(
      rclcpp::get_logger("rclcpp"), "RX timeout/incomplete: %zu / %zu bytes", received,
      expected_rx_size);
    return false;
  }
  ...
  if (rx[2] != servo_id) {
    RCLCPP_WARN(
      rclcpp::get_logger("rclcpp"), "Unexpected servo id: received=%u expected=%u", rx[2],
      servo_id);
    return false;
  }
  ...
  if (length != read_size + 2) {
    RCLCPP_WARN(
      rclcpp::get_logger("rclcpp"), "Unexpected response length: received=%u expected=%u", length,
      static_cast<unsigned>(read_size + 2));
    return false;
  }

  if (error != 0x00) {
    RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "Servo returned error: 0x%02X", error);
    return false;
  }
  ...
  if (received_checksum != expected_checksum) {
    RCLCPP_WARN(
      rclcpp::get_logger("rclcpp"), "Checksum mismatch: received=0x%02X expected=0x%02X",
  ...
  auto u8 = [&](size_t addr) -> uint8_t { return rx[5 + addr]; };

  auto u16 = [&](size_t addr) -> uint16_t {
    return static_cast<uint16_t>(
      static_cast<uint16_t>(rx[5 + addr]) | (static_cast<uint16_t>(rx[5 + addr + 1]) << 8));
  };
...
bool koma::FeetechSerial::write_packet(uint8_t tx[], size_t size)
{
  ...
  if (written < 0) {
    RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Serial write failed: %s", std::strerror(errno));
    return false;
  }
...
bool koma::FeetechSerial::set_reverse(
  int servo_id,
  bool reverse)
{
  auto logger = rclcpp::get_logger("rclcpp");
  ...
  //
  // STS3125:
  // Address 10 = Rotation Direction
  //   0x00 : normal
  //   0x01 : reverse
  //
  constexpr uint8_t addr_direction   = 10;
  ...
  if (!write_u8(addr_direction, direction_value)) {
    RCLCPP_ERROR(logger, "Failed to set direction");
    return false;
  }
