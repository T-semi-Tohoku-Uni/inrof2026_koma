...
koma::FieldMeshMarkerPublisher::FieldMeshMarkerPublisher(const rclcpp::NodeOptions & options)
: Node("map_server", options)
{
  ...
  map_publisher_ = this->create_publisher<visualization_msgs::msg::Marker>(
    "field_marker", rclcpp::QoS(rclcpp::KeepLast(1)).reliable().transient_local());

  timer_ =
    this->create_wall_timer(10s, std::bind(&koma::FieldMeshMarkerPublisher::publish_map, this));
}
