...
using KdTree = nanoflann::KDTreeSingleIndexAdaptor<
  nanoflann::L2_Simple_Adaptor<float, koma::PointCloud>, koma::PointCloud, 2>;
...
private:
  ...
  void expandCluster(
    koma::Point & p, std::vector<koma::Point> & points, std::vector<size_t> & neighbors,
    koma::KdTree & tree, const int cluster_id);
  std::vector<size_t> regionQuery(koma::Point & p, koma::KdTree & tree);

  // delete wall
  double median(std::vector<double> & v);
  std::vector<int> deleteWall(std::unordered_map<int, std::vector<koma::Point>> & clusters);
  ...
  // search
  std::optional<geometry_msgs::msg::Pose> findClosestBall(std::vector<koma::Circle> & ball);
