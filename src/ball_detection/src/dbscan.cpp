...
koma::BallDetect::BallDetect(const rclcpp::NodeOptions & options): Node("ball_detect_node", options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_) {
    // parameter
    ...
    this->subLider_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        "/ldlidar_node/scan", lidarScanQos, std::bind(&koma::BallDetect::lidarCallback, this, std::placeholders::_1)
    );
    ...
    srv_ball_pose_ = this->create_service<inrof2026_koma_type::srv::BallPosition>(
        "ball_detect", std::bind(&koma::BallDetect::ballPoseCallback, this, std::placeholders::_1, std::placeholders::_2)
    );
...
void koma::BallDetect::ballPoseCallback(
    ...
    RCLCPP_INFO(this->get_logger(), "Closest ball is (x, y)=(%f, %f)", response->pose_stamped.pose.position.x, response->pose_stamped.pose.position.y);
}
...
std::optional<geometry_msgs::msg::Pose> koma::BallDetect::detect() {
    ...
    */
    ...
    std::vector<std::pair<int, std::vector<koma::Point>>> ball_clusters = koma::BallDetect::collectBallPoints(
    ...
    */
    ...
    // find closest ball
    std::optional<geometry_msgs::msg::Pose> closest_ball = koma::BallDetect::findClosestBall(ball_position);
    if (!closest_ball) return std::nullopt; // no ball
    // print rviz2
    sensor_msgs::msg::PointCloud2 ball_point_cloud = koma::BallDetect::circle2PointCloud2(ball_position);
...
std::unordered_map<int, std::vector<koma::Point>> koma::BallDetect::dbscan(std::vector<koma::Point> &points, koma::KdTree &tree) {
    int C = 0;
    for (koma::Point &p: points) {
        ...
        if (neighbors.size() < MIN_PTS_) p.setID(koma::ClusterID::NOISE);
        else {
...
std::vector<size_t> koma::BallDetect::regionQuery(
    koma::Point &p, 
    koma::KdTree& tree
) {
...
int koma::Point::getPointID() const {
    return this->pointID_;
}

size_t koma::PointCloud::kdtree_get_point_count() const {
    return points.size();
}
...
float koma::Point::getX() const {
    return x_;
}

float koma::Point::getY() const {
    return y_;
}

int koma::Point::getID() const {
    return clusterID_;
}

void koma::Point::setID(int id) {
    clusterID_ = id;
}
...
double koma::UnitVector::getX() const {
    return x_;
}

double koma::UnitVector::getY() const {
    return y_;
}
...
void koma::Circle::markClosest() {
    this->is_closest_ = true;
}

double koma::Circle::getX() {
    return this->x_;
} 

double koma::Circle::getY() {
    return this->y_;
}

double koma::Circle::getR() {
    return this->r_;
}

bool koma::Circle::isClosest() {
    return this->is_closest_;
}

sensor_msgs::msg::PointCloud2 koma::BallDetect::circle2PointCloud2(std::vector<koma::Circle> ball_position)
{
