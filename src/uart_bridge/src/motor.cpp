...
koma::TwistManager::TwistManager(const rclcpp::NodeOptions & options)
: Node("twist_manager", options)
{
  ...
  std::string device_name = this->declare_parameter<std::string>(
    "device_name", "/dev/serial/by-path/platform-fd500000.pcie-pci-0000:01:00.0-usb-0:1.4:1.2");
...
void koma::TwistManager::send_vel(geometry_msgs::msg::Twist msg)
{
  ...
  for (int i = 0; i < 3; i++) {
    std::memcpy(buf + i * 4, u32_bytes[i].byte, 4);
  }
  buf[12] = '\r';
  buf[13] = '\n';
...
void koma::TwistManager::receive_vel_callback()
{
  ...
  if (n > 0) {
    ...
    while (1) {
      ...
      for (int i = 0; i < 3; i++) {
        U32Bytes u32_byte;
        std::copy_n(frame.begin() + i * 4, 4, u32_byte.byte);
      ...
      geometry_msgs::msg::Twist twist =
        inverseKinematics(cmd_feedback[0], cmd_feedback[1], cmd_feedback[2]);
...
int koma::TwistManager::open_serial(const char * device_name)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Serial opened: %s @ 115200, 8N1, raw", device_name);
  return fd;
}
